...
业务端“账单试算”页面可选生效订阅和账期，核对基础费、已用量、包含量、超额量及预计应收；也可调用只读接口 `GET /api/subscriptions/{id}/bill-preview?periodStart=YYYY-MM-DD&periodEnd=YYYY-MM-DD`。试算与正式出账共用计算规则，但不生成账单或审计流水；已出账重叠账期和非生效订阅会被拒绝。当前规则按套餐月价计算基础费，不做按天折算，企业上线前须确认合同计费口径。
...
- 账单与收款：生成费用明细、应收金额；支持分笔到账、累计已收和未收余额，拒绝重复流水号、超额收款及超过两位小数的金额；仅收齐后标记已收款。
